...
cities = generate_cities(n_cities, minx=window_padding, maxx= window_width-window_padding, miny= window_padding, maxy=window_height-window_padding)
population = generate_population(cities, population_size)



from tkinter import Tk, Canvas, Frame, Label, StringVar, W, BOTH, CENTER, SUNKEN, NW
...
class CityMapper(Frame):
    ...
    def __init__(self):
        ...
        self.text = StringVar()
        self.statusbar = Label(self,textvariable=self.text, bd=1, relief=SUNKEN, anchor=W)


    def draw_city(self, x, y, name, **kwargs):
        r = 5
        self.canvas.create_oval(x-r, y-r, x+r, y+r, fill="blue", outline="#DDD", width=4)
        self.canvas.create_text(x, y-10, anchor=CENTER, font=("Open Sans", 10),
            text=name)
    ...
    def draw_best_ui(self, cities, route, generation):
        ...
        self.canvas.create_text(0, 0, anchor=NW, font=("Open Sans", 40),
